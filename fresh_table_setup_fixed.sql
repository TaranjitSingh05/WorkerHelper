-- Fresh WorkerHelper Table Setup - FIXED VERSION
...
-- Create backup of existing data (if any) with timestamp
DO $$
DECLARE
    backup_table_name TEXT;
BEGIN
    backup_table_name := 'workers_data_backup_' || to_char(NOW(), 'YYYYMMDD_HH24MI');
    
    EXECUTE format('CREATE TABLE %I AS SELECT * FROM workers_data', backup_table_name);
    
    RAISE NOTICE 'Backup table created: %', backup_table_name;
EXCEPTION
    WHEN undefined_table THEN
        RAISE NOTICE 'No existing workers_data table to backup';
    WHEN OTHERS THEN
        RAISE NOTICE 'Error creating backup: %', SQLERRM;
END $$;
